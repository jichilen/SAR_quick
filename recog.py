...
class Recogniton(nn.Module):
    ...
    def __init__(self):
        ...
        self.hidden_dim_de = 512
        self.out_seq_len = 30  # only set for eval mode, will change in train mode
    ...
    def forward(self, conv_f, target=None):  # [-1, 512, 6, 40] | -1 30
        ...
        x, _ = self.encode_lstm(
            x, self.hidden_en)  # [40, -1, 512]
        return x,1

        # print(self.hidden_en.shape)
        holistic_feature = x  # [-1 512]
        # decode model
        if self.training:
            assert self.out_seq_len == target.shape[1]
